...
function renderThrowList() {
  ...
  currentThrows.forEach(t => {
    if (!bestByImpl[t.implement] || t.dist > bestByImpl[t.implement]) {
      bestByImpl[t.implement] = t.dist;
    }
  });
  const overallBest = Math.max(...currentThrows.map(t => t.dist));

  // Group by implement for display
  const byImpl = {};
  currentThrows.forEach((t, i) => {
    if (!byImpl[t.implement]) byImpl[t.implement] = [];
    byImpl[t.implement].push({ ...t, idx: i });
  });

  let html = '';
  Object.entries(byImpl).forEach(([impl, throws]) => {
    const label = IMPLEMENT_LABELS[impl] || impl;
    html += `<div style="font-size:11px;letter-spacing:0.08em;text-transform:uppercase;color:var(--text3);margin:10px 0 4px;">${label}</div>`;
    throws.forEach(t => {
      const isBestImpl = t.dist === bestByImpl[impl];
      const isOverall  = t.dist === overallBest;
      html += `
        <div class="throw-entry">
          ...
          <div class="throw-meta">
            ${capitalize(t.type)}${isOverall ? ' 🏆' : isBestImpl ? ' ★' : ''}
            ${t.note ? '<br>' + t.note : ''}
          </div>
          <button class="throw-delete" onclick="removeThrow(${t.idx})">✕</button>
        </div>
      `;
    });
  });
...
function saveSession() {
  ...
  // Compute best per implement
  const implBests = {};
  currentThrows.forEach(t => {
    if (!implBests[t.implement] || t.dist > implBests[t.implement]) implBests[t.implement] = t.dist;
  });
  ...
  const session = {
    ...
    implementBests: implBests,
    implements: [...new Set(currentThrows.map(t => t.implement))],
...
function renderHistory() {
  ...
  container.innerHTML = sessions.map(s => {
    ...
    // Build per-implement best lines for history card
    const implLines = s.implementBests
      ? Object.entries(s.implementBests)
          .map(([impl, best]) => `${IMPLEMENT_LABELS[impl] || impl}: <strong>${best.toFixed(2)}m</strong>`)
          .join(' &nbsp;·&nbsp; ')
      : `${s.bestThrow.toFixed(2)}m`;

    // Implement tags for subtitle
    const implTags = s.implements
      ? s.implements.map(i => IMPLEMENT_LABELS[i] || i).join(' + ')
      : (s.implement || '').replace('_', ' ');
    ...
    return `
      <div class="history-item">
        <div class="history-header">
          <div>
            ...
            <div class="history-sub">
              ${typeLabels[s.sessionType] || s.sessionType} · ${s.throws.length} throws · ${implTags}
            </div>
          </div>
        ...
        ${s.implements && s.implements.length > 1
          ? `<div style="font-size:12px;color:var(--text3);margin-bottom:6px;">${implLines}</div>`
          : ''}
