...
function buildDataSummary() {
  ...
  // Per-implement PR summary
  const implAllBests = {};
  sessions.forEach(s => {
    if (s.implementBests) {
      Object.entries(s.implementBests).forEach(([impl, best]) => {
        if (!implAllBests[impl] || best > implAllBests[impl]) implAllBests[impl] = best;
      });
    }
  });
  if (Object.keys(implAllBests).length > 1) {
    const implLabels = {'16lb_6kg':'6kg','shot_7.26':'7.26kg','shot_5kg':'5kg','shot_4kg':'4kg','disc_2kg':'Disc 2kg','disc_1.6kg':'Disc 1.6kg','disc_1kg':'Disc 1kg'};
    const implStr = Object.entries(implAllBests).map(([k,v]) => `${implLabels[k]||k}: ${v.toFixed(2)}m`).join(', ');
    lines.push(`All-time PR per implement: ${implStr}`);
  }
